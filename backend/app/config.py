...
class Settings(BaseSettings):
    ...
    # -------------------------------------------------------------------------
    llm_provider: Literal["ollama", "gemini", "groq"] = Field(
        default="gemini",
        description="LLM provider to use: 'ollama' for local models, 'gemini' for Google Gemini API, 'groq' for Groq API"
    )
    ...
    weight_hallucination: float = Field(default=0.40)
    weight_logical_inconsistency: float = Field(default=0.20)
    weight_missing_assumptions: float = Field(default=0.15)
    weight_overconfidence: float = Field(default=0.15)
