...
async def hallucination_detector(state: AnalysisState) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info("hallucination_detector.llm_result",
                     parse_error=result.get("_parse_error", False),
                     detected=result.get("hallucination_detected"),
        ...
        logger.error("hallucination_detector.error", error=str(e), error_type=type(e).__name__)
        signal = _create_default_signal()
        return {
            "hallucination_signal": signal,
