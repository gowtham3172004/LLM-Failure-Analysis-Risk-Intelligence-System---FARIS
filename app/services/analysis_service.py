...
class AnalysisService:
    ...
    def _build_response(
        ...
        # Determine which model was used based on provider
        model_used = settings.gemini_model if settings.llm_provider == "gemini" else settings.ollama_model
        ...
        remediation = None
        if result.get("remediation_attempted"):
            remediation = RemediationResult(
                attempted=True,
                corrected_answer=result.get("remediated_answer"),
                explanation=result.get("remediation_explanation"),
            )
        elif result.get("remediation_attempted") is False:
            remediation = RemediationResult(attempted=False)

        # Build warnings from pipeline errors
        warnings = []
        rate_limited = False
        for err in result.get("errors", []):
            node = err.get("node", "unknown")
            error_msg = err.get("error", "")
            if "rate limit" in error_msg.lower() or "429" in error_msg.lower() or "quota" in error_msg.lower() or "RateLimitError" in error_msg:
                rate_limited = True
            elif error_msg:
                warnings.append(f"Error in {node}: {error_msg}")
        
        if rate_limited:
            warnings.insert(0, "Some analysis steps were skipped due to LLM API rate limits. Results may be incomplete — please retry in a few minutes for full analysis.")

        # Override explanation if most of the pipeline was rate-limited
        final_explanation = result.get("explanation", "Analysis complete.")
        if rate_limited and not result.get("failure_detected", False):
            final_explanation = (
                "Analysis was partially completed due to LLM API rate limits. "
                "Some failure detectors could not run, so this result may not reflect "
                "all issues. Please retry in a few minutes for complete analysis."
            )
    ...
    async def _persist_analysis(
        ...
        try:
            ...
            analysis_model = settings.gemini_model if settings.llm_provider == "gemini" else settings.ollama_model
